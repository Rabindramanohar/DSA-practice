class Solution {
    ...
    int helper(int[] houses, int[][] cost, int targetCount, int currIndex,
        ...
        if (houses[currIndex] != 0) {
            ...
            minCost = 
                helper(houses, cost, targetCount, currIndex + 1, newNeighborhoodCount, houses[currIndex]);
        } else {
            ...
            for (int color = 1; color <= totalColors; color++) {
                ...
                int currCost = cost[currIndex][color - 1] 
                    + helper(houses, cost, targetCount, currIndex + 1, newNeighborhoodCount, color);
    ...
    public int minCost(int[] houses, int[][] cost, int m, int n, int target) {
        int answer = helper(houses, cost, target, 0, 0, 0);
